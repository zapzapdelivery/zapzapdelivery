...
import styles from './novo-entregador.module.css';
...
export default function NovoEntregadorPage() {
  ...
  useEffect(() => {
    if (!loadingRole) {
      ...
      // Se for estabelecimento, preenche o ID automaticamente
      if (establishmentId && !formData.estabelecimento_id) {
        setFormData(prev => ({ ...prev, estabelecimento_id: establishmentId }));
      }
    }
  }, [role, establishmentId, loadingRole, router, formData.estabelecimento_id]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Criar o entregador na tabela entregadores
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao salvar entregador');
      }

      success('Entregador e conta de acesso criados com sucesso!');
      router.push('/entregadores');
    } catch (err: any) {
      console.error('Error saving:', err);
      toastError(err.message || 'Erro ao salvar entregador');
    } finally {
      setLoading(false);
    }
  };
